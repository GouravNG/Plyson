...
const hashPassword = p => crypto.createHash("sha256").update(CONFIG.HASH_SALT + p).digest("hex");
...
function isRateLimited(ip) {
  const now = Date.now(), cutoff = now - CONFIG.RATE_WINDOW_MS;
...
setInterval(() => {
  ...
  for (const [ip, hits] of _rateMap)
    if (hits.every(t => t <= cutoff)) _rateMap.delete(ip);
}, 5 * 60_000).unref();
...
function validate(data, schema) {
  ...
  for (const [field, rules] of Object.entries(schema)) {
    ...
    if (rules.required && missing) { errors.push(`'${field}' is required`); continue; }
    if (missing) continue;
...
function paginate(array, query) {
  ...
  return { data: array.slice((page - 1) * limit, page * limit), meta: { page, limit, total, pages: Math.ceil(total / limit) || 1 } };
}
...
const softDel = r => { r.deletedAt = new Date().toISOString(); return r; };
const liveList = (arr, query) => query && query.includeDeleted === "true" ? arr : arr.filter(isAlive);
...
function pushNotification(userId, type, title, body) {
  DB.notifications.push({
    id: nextId(), userId, type, title, body,
...
const ok = (res, data, reqId, status = 200) => send(res, status, { success: true, ...(data || {}) }, reqId);
const err = (res, message, reqId, status = 400, extra = {}) => send(res, status, { success: false, error: message, ...extra }, reqId);
...
function resolveUser(req) {
  ...
  if (Date.now() > session.expiresAt) { delete DB.sessions[token]; return { user: null, token, reason: "token_expired" }; }
...
function requireAuth(res, req, reqId) {
  ...
  if (!user) err(res, reason === "token_expired" ? "Token expired — please log in again" : "Unauthorized — valid Bearer token required", reqId, 401);
  return user;
}
...
function requireRole(res, req, role, reqId) {
  ...
  if (!user) return null;
  if (user.role !== role) { err(res, `Forbidden — requires role: ${role}`, reqId, 403); return null; }
  return user;
}
...
route("POST", "/auth/register", async (req, res, { reqId }) => {
  ...
  DB.users.push(user);
  ok(res, { message: "Registered — account is INACTIVE until approved", user: safeUser(user) }, reqId, 201);
});
...
route("POST", "/auth/login", async (req, res, { reqId }) => {
  ...
  const email = body.email.toLowerCase().trim();
  const user = DB.users.find(u => u.email === email && u.passwordHash === hashPassword(body.password) && !u.deletedAt);
  ...
  ok(res, { message: "Login successful", token, expiresAt: new Date(expiresAt).toISOString(), user: safeUser(user) }, reqId);
});
...
route("POST", "/auth/refresh", async (req, res, { reqId }) => {
  ...
  if (!user) return err(res, reason === "token_expired" ? "Token expired — please log in again" : "Unauthorized", reqId, 401);
  DB.sessions[token].expiresAt = Date.now() + CONFIG.TOKEN_TTL_MS;
  ok(res, { message: "Token refreshed", expiresAt: new Date(DB.sessions[token].expiresAt).toISOString() }, reqId);
});
...
route("POST", "/user", async (req, res, { reqId }) => {
  ...
  const email = body.email.toLowerCase().trim();
  if (DB.users.find(u => u.email === email && !u.deletedAt)) return err(res, "Email already in use", reqId, 409);

  const now = new Date().toISOString();
  const nu = { id: nextId(), email, passwordHash: hashPassword(body.password), firstname: body.firstname.trim(), lastname: body.lastname.trim(), role: "USER", status: "INACTIVE", createdAt: now, updatedAt: now, deletedAt: null };
  DB.users.push(nu);
...
route("POST", "/foundation/business", async (req, res, { reqId }) => {
  ...
  if (!valid) return err(res, "Validation failed", reqId, 422, { errors });
  const now = new Date().toISOString();
  const biz = { id: nextId(), name: body.name.trim(), email: body.email || null, phone: body.phone || null, address: body.address || null, createdAt: now, updatedAt: now, deletedAt: null };
...
route("DELETE", /^\/foundation\/business\/(?<id>[^/]+)$/, async (req, res, { params, reqId }) => {
  ...
  const biz = DB.businesses.find(b => b.id === params.id && !b.deletedAt);
  if (!biz) return err(res, "Business not found", reqId, 404);
  softDel(biz); ok(res, { message: "Business deleted", business: biz }, reqId);
});
...
route("POST", "/foundation/store", async (req, res, { reqId }) => {
  ...
  const now = new Date().toISOString();
  const store = { id: nextId(), name: body.name.trim(), businessId: body.businessId || null, location: body.location || null, createdAt: now, updatedAt: now, deletedAt: null };
...
route("DELETE", /^\/foundation\/store\/(?<id>[^/]+)$/, async (req, res, { params, reqId }) => {
  ...
  const store = DB.stores.find(s => s.id === params.id && !s.deletedAt);
  if (!store) return err(res, "Store not found", reqId, 404);
  softDel(store); ok(res, { message: "Store deleted", store }, reqId);
});
...
route("POST", "/foundation/services", async (req, res, { reqId }) => {
  ...
  if (!valid) return err(res, "Validation failed", reqId, 422, { errors });
  const now = new Date().toISOString();
  const svc = { id: nextId(), name: body.name.trim(), description: body.description || null, price: body.price ?? null, createdAt: now, updatedAt: now, deletedAt: null };
...
route("DELETE", /^\/foundation\/services\/(?<id>[^/]+)$/, async (req, res, { params, reqId }) => {
  ...
  const svc = DB.services.find(s => s.id === params.id && !s.deletedAt);
  if (!svc) return err(res, "Service not found", reqId, 404);
  softDel(svc); ok(res, { message: "Service deleted", service: svc }, reqId);
});

// ── ASSOCIATION ───────────────────────────────────────────────────────────────

route("GET", /^\/foundation\/association\/(?<storeId>[^/]+)$/, async (req, res, { params, reqId }) => {
  if (!requireRole(res, req, "ADMIN", reqId)) return;
  if (!DB.stores.find(s => s.id === params.storeId && !s.deletedAt)) return err(res, "Store not found", reqId, 404);
  const services = DB.associations.filter(a => a.storeId === params.storeId)
    .map(a => DB.services.find(s => s.id === a.serviceId && !s.deletedAt)).filter(Boolean);
  ok(res, { storeId: params.storeId, services }, reqId);
});
route("POST", /^\/foundation\/association\/(?<storeId>[^/]+)$/, async (req, res, { params, reqId }) => {
  if (!requireRole(res, req, "ADMIN", reqId)) return;
  if (!DB.stores.find(s => s.id === params.storeId && !s.deletedAt)) return err(res, "Store not found", reqId, 404);
  const body = await parseBody(req);
  let svc = body.serviceId ? DB.services.find(s => s.id === body.serviceId && !s.deletedAt) : null;
  if (!svc && body.name) {
    const now = new Date().toISOString();
    svc = { id: nextId(), name: body.name.trim(), description: body.description || null, price: body.price ?? null, createdAt: now, updatedAt: now, deletedAt: null };
    DB.services.push(svc);
  }
  ...
  DB.associations.push({ storeId: params.storeId, serviceId: svc.id, createdAt: new Date().toISOString() });
  ok(res, { message: "Service associated", storeId: params.storeId, service: svc }, reqId, 201);
});
route("DELETE", /^\/foundation\/association\/(?<storeId>[^/]+)\/(?<serviceId>[^/]+)$/, async (req, res, { params, reqId }) => {
  if (!requireRole(res, req, "ADMIN", reqId)) return;
  const idx = DB.associations.findIndex(a => a.storeId === params.storeId && a.serviceId === params.serviceId);
...
function enrichCart(cart) {
  const items = cart.items.map(i => ({ ...i, service: DB.services.find(s => s.id === i.serviceId && !s.deletedAt) || null }));
...
route("POST", "/cart", async (req, res, { reqId }) => {
  ...
  if (!user) return;
  const body = await parseBody(req);
  const { valid, errors } = validate(body, { serviceId: { required: true, type: "string" }, qty: { type: "number", min: 1 } });
  if (!valid) return err(res, "Validation failed", reqId, 422, { errors });
  if (!DB.services.find(s => s.id === body.serviceId && !s.deletedAt)) return err(res, "Service not found", reqId, 404);
  ...
  if (ex) { ex.qty += qty; } else { cart.items.push({ serviceId: body.serviceId, qty, addedAt: new Date().toISOString() }); }
...
route("PATCH", "/cart", async (req, res, { reqId }) => {
  ...
  if (!user) return;
  const body = await parseBody(req);
  const { valid, errors } = validate(body, { serviceId: { required: true, type: "string" }, qty: { required: true, type: "number", min: 0 } });
  if (!valid) return err(res, "Validation failed", reqId, 422, { errors });
  const cart = getUserCart(user.id);
  const item = cart.items.find(i => i.serviceId === body.serviceId);
  if (!item) return err(res, "Item not in cart", reqId, 404);
  if (body.qty === 0) { cart.items = cart.items.filter(i => i.serviceId !== body.serviceId); } else { item.qty = Math.round(body.qty); }
  cart.updatedAt = new Date().toISOString();
  ok(res, { cart: enrichCart(cart) }, reqId);
});
route("DELETE", "/cart", async (req, res, { reqId }) => {
  ...
  const cart = getUserCart(user.id);
  if (body.serviceId) {
    if (!cart.items.find(i => i.serviceId === body.serviceId)) return err(res, "Item not in cart", reqId, 404);
...
function buildOrderItems(cartItems) {
  return cartItems.map(i => {
    ...
    return { serviceId: i.serviceId, serviceName: svc.name || "Unknown", price: svc.price || 0, qty: i.qty, lineTotal: (svc.price || 0) * i.qty };
  });
}
...
route("POST", "/orders", async (req, res, { reqId }) => {
  ...
  const cart = getUserCart(user.id);
  if (!cart.items.length) return err(res, "Cart is empty — add items before checking out", reqId, 400);
  ...
  cart.items = []; cart.updatedAt = now;

  pushNotification(user.id, "ORDER", "Order placed", `Your order #${order.id} has been placed and is PENDING.`);
...
route("POST", /^\/orders\/(?<id>[^/]+)\/cancel$/, async (req, res, { params, reqId }) => {
  ...
  order.statusHistory.push({ status: "CANCELLED", at: now, by: user.id, reason: order.cancelReason });
  order.updatedAt = now;
...
route("PATCH", /^\/orders\/(?<id>[^/]+)\/status$/, async (req, res, { params, reqId }) => {
  ...
  if (!ORDER_STATUSES.includes(target)) return err(res, `Invalid status. Valid: ${ORDER_STATUSES.join(", ")}`, reqId, 400);
  if (!ORDER_TRANSITIONS[order.status]?.includes(target))
    return err(res, `Cannot transition from ${order.status} → ${target}. Allowed: ${ORDER_TRANSITIONS[order.status].join(", ") || "none"}`, reqId, 409);
  ...
  pushNotification(order.userId, "ORDER", "Order update", `Your order #${order.id} is now ${target}.`);
...
route("POST", "/reviews", async (req, res, { reqId }) => {
  ...
  if (!order) return err(res, "Order not found", reqId, 404);
  if (order.status !== "DELIVERED") return err(res, "You can only review services from DELIVERED orders", reqId, 409);
  ...
  const now = new Date().toISOString();
  const review = {
    id: nextId(), userId: user.id, serviceId: body.serviceId, orderId: body.orderId,
    rating: Math.round(body.rating), comment: body.comment || null,
...
route("PATCH", /^\/reviews\/(?<id>[^/]+)\/moderate$/, async (req, res, { params, reqId }) => {
  ...
  pushNotification(review.userId, "REVIEW",
...
route("GET", "/notifications", async (req, res, { query, reqId }) => {
  ...
  let list = user.role === "ADMIN" ? DB.notifications : DB.notifications.filter(n => n.userId === user.id);
...
route("DELETE", /^\/notifications\/(?<id>[^/]+)$/, async (req, res, { params, reqId }) => {
  const user = requireAuth(res, req, reqId);
  if (!user) return;
  const idx = DB.notifications.findIndex(n => n.id === params.id && (n.userId === user.id || user.role === "ADMIN"));
...
route("POST", "/notifications/broadcast", async (req, res, { reqId }) => {
  ...
  const targets = DB.users.filter(u => !u.deletedAt && (!body.role || u.role === body.role.toUpperCase()));
...
route("GET", "/", async (req, res, { reqId }) => {
  ok(res, {
    name: "Mock API Server", version: CONFIG.VERSION,
    ...
    controllers: {
      ...
      FOUNDATION: ["CRUD /foundation/business[/:id]", "CRUD /foundation/store[/:id]", "CRUD /foundation/services[/:id]", "GET|POST /foundation/association/:storeId", "DELETE /foundation/association/:s/:svcId"],
      CART: ["GET|POST|PATCH|DELETE /cart"],
      ORDER: ["POST /orders", "GET /orders", "GET /orders/:id", "POST /orders/:id/cancel", "PATCH /orders/:id/status [ADMIN]"],
      REVIEW: ["POST /reviews", "GET /reviews", "GET|PATCH|DELETE /reviews/:id", "PATCH /reviews/:id/moderate [ADMIN]"],
      NOTIFICATION: ["GET /notifications", "PATCH /notifications/:id/read", "POST /notifications/read-all", "DELETE /notifications/:id", "POST /notifications/broadcast [ADMIN]"],
      UTILITY: ["GET /", "GET /health [ADMIN]", "POST /cache-clear [ADMIN]", "GET /db-snapshot [ADMIN]", "GET /openapi.json", "GET /docs"],
    },
  }, reqId);
});
...
route("GET", "/health", async (req, res, { reqId }) => {
  if (!requireRole(res, req, "ADMIN", reqId)) return;
  const mem = process.memoryUsage(), toMB = b => `${(b / 1024 / 1024).toFixed(2)} MB`;
  ok(res, {
    status: "healthy", uptimeSec: Math.floor(process.uptime()),
    memory: { rss: toMB(mem.rss), heapUsed: toMB(mem.heapUsed), heapTotal: toMB(mem.heapTotal) },
    config: { port: CONFIG.PORT, tokenTtl: `${CONFIG.TOKEN_TTL_MS / 3_600_000}h`, rateLimit: `${CONFIG.RATE_LIMIT} req/min per IP` },
    db: {
      users: DB.users.filter(isAlive).length, usersDeleted: DB.users.filter(u => u.deletedAt).length,
      activeSessions: Object.keys(DB.sessions).length,
      businesses: DB.businesses.filter(isAlive).length, stores: DB.stores.filter(isAlive).length,
      services: DB.services.filter(isAlive).length, associations: DB.associations.length,
      carts: DB.carts.length, orders: DB.orders.length,
      reviews: DB.reviews.length, notifications: DB.notifications.length,
    },
    timestamp: new Date().toISOString(),
  }, reqId);
});
...
route("POST", "/cache-clear", async (req, res, { reqId }) => {
  ...
  ok(res, { message: "All sessions invalidated", sessionsCleared: count, timestamp: new Date().toISOString() }, reqId);
});
...
route("GET", "/db-snapshot", async (req, res, { reqId }) => {
  ...
  ok(res, {
    ...
    snapshot: {
      ...
      sessions: Object.entries(DB.sessions).map(([t, s]) => ({ tokenPrefix: t.slice(0, 8) + "…", userId: s.userId, expiresAt: new Date(s.expiresAt).toISOString(), expired: Date.now() > s.expiresAt })),
      businesses: DB.businesses, stores: DB.stores, services: DB.services,
      associations: DB.associations, carts: DB.carts,
      orders: DB.orders, reviews: DB.reviews, notifications: DB.notifications,
    },
  }, reqId);
});
...
route("GET", "/openapi.json", async (req, res, { reqId }) => {
  ...
  res.writeHead(200, { "Content-Type": "application/json", "Content-Length": Buffer.byteLength(body), "X-Request-Id": reqId });
  res.end(body);
});
...
const server = http.createServer(async (req, res) => {
  ...
  if (isRateLimited(ip)) {
    send(res, 429, { success: false, error: `Rate limit exceeded — max ${CONFIG.RATE_LIMIT} req/min`, retryAfterMs: CONFIG.RATE_WINDOW_MS }, reqId);
  ...
  let _status = 200;
  res.writeHead = (code, ...rest) => { _status = code; return _wh(code, ...rest); };

  try {
    await match.handler(req, res, { ...match, reqId });
  } catch (e) {
    console.error(`\n${C.red}[UNHANDLED]${C.reset} ${e.stack}\n`);
    if (!res.headersSent) { send(res, 500, { success: false, error: "Internal server error" }, reqId); _status = 500; }
  }
