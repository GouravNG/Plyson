...
You can also control whether suites run in parallel or sequentially through the `mode` field in [project.json](/docs/test/json-reference/project) or a [suite file](/docs/test/json-reference/test-suites). `parallel` is the default for project-level execution, while `sequential` is useful for stateful flows.
