...
Refine and complete the CLI implementation to provide a professional, interactive experience for managing and running `plyson` projects. This phase ensures the CLI matches the design specifications in `CLI.md` and provides robust utility for test authors.
...
- **E2E Tests**: Confirmed that `plyson run` correctly executes all scenarios in the E2E project fixture.
...
```bash
plyson run --env staging --grep "@smoke" --workers 1
```

### Interactively fix project issues

```bash
plyson validate --repair
```
...
```bash
plyson g env-var api_key "SECRET_123" --env prod
```

## Status

All phases of the `plyson` framework are now complete. The framework is ready for declarative API testing at scale.
