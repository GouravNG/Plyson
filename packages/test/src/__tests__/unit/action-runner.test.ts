...
describe('ActionRunner', () => {
  it('should execute a valid action with context', async () => {
    ...
    const actions = new Map([['hello', { default: actionFn }]])
  ...
  it('should resolve variables in arguments before calling action', async () => {
    ...
    const actions = new Map([['greet', { default: actionFn }]])
  ...
  it('should throw error if action is not found', async () => {
    ...
    await expect(runner.runAction({ title: 'Fail', action: 'missing' })).rejects.toThrow(
      'Action "missing" not found',
    )
  })

  it('should throw error if action has no default export', async () => {
    const actions = new Map([['invalid', {}]])
    const runner = new ActionRunner(
      {} as any,
      new VariableStore(),
      actions as any,
    ...
    await expect(runner.runAction({ title: 'Fail', action: 'invalid' })).rejects.toThrow(
