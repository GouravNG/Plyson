  ...
  describe('Issue #7: Ref style testcase not working at Global scope', () => {
    it('should resolve refs in project beforeAll/afterAll', async () => {
        ...
        mockedFs.readFile.mockImplementation((p: string) => {
            if (p.endsWith('project.json')) return JSON.stringify({
                title: 'Test',
                version: '1.0.0',
            ...
            if (p.endsWith('setup-db.script.json')) return JSON.stringify({
                id: 'setup-db',
                title: 'Setup DB',
                tags: [],
                steps: [{ title: 'Init', request: { method: 'GET', endpoint: '/init' }, response: { validations: { statusCode: 200 } } }]
            })
            return '[]'
        })
