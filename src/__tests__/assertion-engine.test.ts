...
describe('AssertionEngine', () => {
  ...
  describe('Operators', () => {
    ...
    it('should throw for not equals', async () => {
      await expect(AssertionEngine.runAssertion(
    ...
    it('should handle containsSubset', async () => {
      await AssertionEngine.runAssertion(
        ...
        { a: 1, b: 2 }, mockResponse, []
      )
    })

    it('should handle type checks', async () => {
      await AssertionEngine.runAssertion({ title: 'str', from: 'body', path: 'a', operator: 'isString' }, { a: 's' }, mockResponse, [])
      await AssertionEngine.runAssertion({ title: 'num', from: 'body', path: 'a', operator: 'isNumber' }, { a: 1 }, mockResponse, [])
      await AssertionEngine.runAssertion({ title: 'bool', from: 'body', path: 'a', operator: 'isBoolean' }, { a: true }, mockResponse, [])
      await AssertionEngine.runAssertion({ title: 'arr', from: 'body', path: 'a', operator: 'isArray' }, { a: [] }, mockResponse, [])
      await AssertionEngine.runAssertion({ title: 'obj', from: 'body', path: '$', operator: 'isObject' }, { a: 1 }, mockResponse, [])
    })

    it('should throw for isObject with null or array', async () => {
        await expect(AssertionEngine.runAssertion({ title: 'obj', from: 'body', path: 'a', operator: 'isObject' }, { a: null }, mockResponse, [])).rejects.toThrow()
        await expect(AssertionEngine.runAssertion({ title: 'obj', from: 'body', path: 'a', operator: 'isObject' }, { a: [] }, mockResponse, [])).rejects.toThrow()
    })
  })
  ...
  describe('Soft Errors', () => {
    it('should push to softErrors when validation is warn', async () => {
      ...
      await AssertionEngine.runAssertion(
        { title: 'warn me', from: 'body', path: 'a', operator: 'equals', value: 2, validation: 'warn' },
        { a: 1 }, mockResponse, softErrors
      )
